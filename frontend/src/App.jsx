...
Protected.propTypes = { children: PropTypes.node.isRequired, roles: PropTypes.arrayOf(PropTypes.string) }
Public.propTypes    = { children: PropTypes.node.isRequired }

function AuthSync() {
  const dispatch = useDispatch()
  const isAuth   = useSelector(selIsAuth)
  useEffect(() => { if (isAuth) dispatch(fetchMe()) }, [isAuth, dispatch])
  return null
}
  ...
  return (
    <BrowserRouter>
      ...
      <Toaster position="top-right" gutter={8} toastOptions={{
        style: { background: T.colors.bg.elevated, color: T.colors.text.primary, border: `1px solid ${T.colors.bg.border}`, borderRadius: T.radius.md, fontFamily: T.fonts.body, fontSize: '13px', boxShadow: T.shadows.lg, maxWidth: '400px' },
        success: { iconTheme: { primary: T.colors.teal.DEFAULT,    secondary: T.colors.bg.elevated } },
        error:   { iconTheme: { primary: T.colors.danger.text,     secondary: T.colors.bg.elevated } },
        loading: { iconTheme: { primary: T.colors.primary.DEFAULT, secondary: T.colors.bg.elevated } },
        duration: 3500,
      }} />
      <AppRoutes />
    </BrowserRouter>
